...
def log_coverage(state):
    ...
    if op not in OPCODES:
        ...
        if NEW_NEW_COVERAGE:
            log(
                "OP: {} Typehashes:{} CoverageOldTypehash:{} CoverageNewTypehash:{}".format(
                    op,
                    len(OPCODES_TYPEHASHES[op]),
                    OPCODES_COVERAGE_OLDT[op],
                    OPCODES_COVERAGE_NEWT[op],
                )
            )
...
def run(code, depth=50):
    ...
    while (
        not has_returned
        and engine._VMState & VMState.HALT == 0
        and engine._VMState & VMState.FAULT == 0
    ):
        # try:
                    ...
                    if type(item) == InteropService.Array:
                        ...
                        if len(arr) == 2:
                            ...
                            if type(ret_id) == neo.ByteArray:
                                ...
                                out = (
                                    b"ret,"
                                    + struct.pack("II", ret_id.GetBigInteger(), len(b))
                                    + b
                                )
